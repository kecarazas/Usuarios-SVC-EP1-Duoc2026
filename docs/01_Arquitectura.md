# Usuarios — Arquitectura del microservicio

## Componentes

| Componente | Rol |
|---|---|
| `UsuarioApplication` | Punto de entrada Spring Boot |
| `controller/UsuarioController.java` | API REST expuesta en `/api/usuarios` |
| `service/UsuarioService.java` | Lógica de negocio y transacciones |
| `repository/UsuarioRepository.java` | Acceso JPA a H2 |
| `model/Usuario.java` | Entidad persistida en tablas H2 |
...
`Cliente/Sistema → API Gateway → UsuarioController → UsuarioService → UsuarioRepository → H2`
